...
2. **Konfigurieren → API → Einstellungen:**
   ...
   - Socket Port: `4002`
   - Vertrauenswürdige IPs: WSL2-IP eintragen (siehe unten)
...
```bash
curl -LsSf https://raw.githubusercontent.com/nabussan/deerflow_portfolio/main/install.sh | bash
```
...
./start.sh
tail -f /tmp/langgraph.log /tmp/gateway.log /tmp/frontend.log
```
